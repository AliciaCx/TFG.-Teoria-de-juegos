...
$\sigma^*_i=(\sigma^{1*}_i , \sigma^{2*}_i , ..., \sigma^{j*}_i , ...)$ tener $\sigma^{j*}_i > 0$ implica que la estrategia pura $s^j_i$ es una respuesta óptima a $\sigma^*_{-i} = (\sigma^*_1 , ..., \sigma^*_{i-1}, \sigma^*_{i+1}, ..., \sigma^*_n)$.
...
Si G es un juego dinámico finito (número finito de jugadores, cada uno de ellos con un conjunto finito de estrategias), existe un equilibrio de Nash perfecto en subjuegos de G.
...
**Teorema 3.4**
Cada juego finito con información completa y perfecta G tiene un Equilibrio de Nash Perfecto en Subjuegos (ENPS) en estrategias puras que se obtiene por el método de inducción hacia atrás. Además, si ningún jugador tiene más de una acción óptima en cada nodo de decisión, tal ENPS es único.
