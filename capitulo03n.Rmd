...
En el juego $G = \{S_1, \dots, S_n; u_1, \dots, u_n\}$, la combinación de estrategias mixtas 
$p^* = (p_1^*, p_2^*, \dots, p_n^*)$ es un equilibrio de Nash si y sólo si para cada jugador $i$, 
tener $p_{i}^{j*} > 0$ implica que la estrategia pura $s_i^j$ es una respuesta óptima a 
$p_{-i}^* = (p_1^*, \dots, p_{i-1}^*, p_{i+1}^*, \dots, p_n^*)$.

\medskip

\textbf{Demostración:}

$(\Rightarrow)$ Supongamos que el perfil $p^* = (p_1^*, p_2^*, \dots, p_n^*)$ cumple, para cada jugador $i$, 
que todas sus estrategias puras $s_i^j$ en el soporte de $p_i^*$ son respuesta óptima a $p_{-i}^*$.

Llamemos $M_i^*$ a la máxima utilidad que puede ser alcanzada por el jugador $i$ si los demás juegan $p_{-i}^*$, es decir,
\[
M_i^* = \max \{ u_i(p_1^*, \dots, p_{i-1}^*, s_i, p_{i+1}^*, \dots, p_n^*) : s_i \in S_i \}.
\]

En ese caso,
\[
u_i(p_1^*, \dots, p_{i-1}^*, s_i^j, p_{i+1}^*, \dots, p_n^*) = M_i^*, 
\quad \forall s_i^j \in \operatorname{supp}(p_i^*).
\]

Y, en consecuencia,
\[
u_i(p_1^*, \dots, p_{i-1}^*, p_i^*, p_{i+1}^*, \dots, p_n^*) 
= \sum_{j: \, p_i^{j*} > 0} p_i^{j*} \, 
u_i(p_1^*, \dots, p_{i-1}^*, s_i^j, p_{i+1}^*, \dots, p_n^*)
\]
\[
= \sum_{j: \, p_i^{j*} > 0} p_i^{j*} \, M_i^* = M_i^*.
\]

Por otra parte, para cualquier $p_i \in \Delta(S_i)$,
\[
u_i(p_1^*, \dots, p_{i-1}^*, p_i, p_{i+1}^*, \dots, p_n^*) \le M_i^*.
\]

Así pues, $p_i^*$ es respuesta óptima a $p_{-i}^*$ para todo jugador $i$, por lo que $p^*$ es un equilibrio de Nash.
\end{minipage}
}
\end{center}
